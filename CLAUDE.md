...
- JavaScript host-emulation config mirrors esbuild's vocabulary so users carry over a known mental model. The host environment we present to guest JS is a `platform`, and its values are esbuild's exactly: `node` | `browser` | `neutral` (Node host surface / web-platform surface / language-only — never invent `web`/`bare`). Wherever a JS runtime/resolution config property has an esbuild equivalent, take esbuild's name and value spelling over any other source (esbuild > tsconfig > ad-hoc); only introduce a non-esbuild property name when esbuild has no equivalent concept.
