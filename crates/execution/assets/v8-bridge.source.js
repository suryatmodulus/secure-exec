...
// packages/build-tools/bridge-src/prelude.ts
...
var NativeAbortControllerGlobal = globalThis.AbortController;
var NativeAbortSignalGlobal = globalThis.AbortSignal;
var EarlyBufferGlobal = bufferStdlibModuleNs.Buffer ?? bufferStdlibModuleNs.default?.Buffer ?? bufferStdlibModuleNs.default;
...
var EarlyUtilTypes = utilStdlibModuleNs.types ?? utilStdlibModuleNs.default?.types;
var StructuredCloneTypedArrayCtors = /* @__PURE__ */ new Map([
...
function sandboxStructuredClone(value, options = void 0) {
  ...
  return cloneStructuredValue(value, /* @__PURE__ */ new Map());
}
...
// packages/build-tools/bridge-src/polyfills.ts
...
if (typeof AbortSignal.any !== "function") {
  Object.defineProperty(AbortSignal, "any", {
    ...
    writable: true,
    value(signals3) {
      if (!signals3 || typeof signals3[Symbol.iterator] !== "function") {
        throw new TypeError('The "signals" argument must be an iterable');
      }
      const inputs = Array.from(signals3);
      const controller = new AbortController();
      if (inputs.length === 0) {
        return controller.signal;
      }
...
if (globalThis.WebAssembly && typeof globalThis.WebAssembly.instantiateStreaming !== "function") {
...
var undiciWebidl = undiciWebidlModule?.webidl ?? undiciWebidlModule;
if (undiciWebidl?.is) {
  undiciWebidl.is.ReadableStream = (value) => value != null && (value instanceof globalThis.ReadableStream || typeof value.getReader === "function");
  undiciWebidl.is.AbortSignal = (value) => value != null && (value instanceof globalThis.AbortSignal || typeof value.aborted === "boolean" && typeof value.addEventListener === "function");
}
if (undiciWebidl?.converters?.AbortSignal) {
  undiciWebidl.converters.AbortSignal = (value, ...args) => {
    if (value != null && (value instanceof globalThis.AbortSignal || typeof value.aborted === "boolean" && typeof value.addEventListener === "function")) {
      return value;
    }
...
// packages/build-tools/bridge-src/global-exposure.ts
...
// packages/build-tools/bridge-src/transport.ts
...
// packages/build-tools/bridge-src/vendor/esbuild-runtime.ts
...
// packages/build-tools/bridge-src/vendor/buffer.ts
...
function toUint8ArrayChunk(chunk, encoding) {
  if (typeof chunk === "string") {
    return import_buffer2.Buffer.from(chunk, encoding ?? "utf8");
  }
  if (import_buffer2.Buffer.isBuffer(chunk)) {
...
var FileHandle = class _FileHandle {
  ...
  async read(buffer, offset, length, position) {
    ...
    if (target === null) {
      target = import_buffer2.Buffer.alloc(FILE_HANDLE_READ_BUFFER_BYTES);
    }
  ...
  async write(buffer, offsetOrPosition, lengthOrEncoding, position) {
    ...
    if (typeof buffer === "string") {
      ...
      const bytesWritten2 = fs.writeSync(handle.fd, import_buffer2.Buffer.from(buffer, encoding), 0, void 0, offsetOrPosition ?? null);
  ...
  async readFile(options) {
    ...
    while (true) {
      throwIfAborted(signal);
      const chunk = import_buffer2.Buffer.alloc(FILE_HANDLE_READ_CHUNK_BYTES);
    ...
    const result = import_buffer2.Buffer.concat(chunks, totalLength);
...
function validateEncodingValue(encoding) {
  ...
  if (typeof encoding !== "string" || !import_buffer2.Buffer.isEncoding(encoding)) {
...
function normalizePathLike(path, name = "path") {
  ...
  if (import_buffer2.Buffer.isBuffer(path)) {
...
function applyProcessUmask(mode) {
  return mode & ~511 | mode & 511 & ~(_umask & 511);
}
...
function createWatcherFilename(path, encoding) {
  ...
  if (encoding === "buffer") {
    return import_buffer2.Buffer.from(basename);
  }
  return basename;
}
  ...
  if (typeof buffer === "string") {
    if (lengthOrEncoding === void 0 && position === void 0 && isReadWriteOptionsObject(offsetOrPosition)) {
      ...
      return {
        buffer,
        offset: 0,
        length: import_buffer2.Buffer.byteLength(buffer, encoding),
        position: normalizeOptionalPosition(options.position),
        encoding
      };
    }
    ...
    return {
      buffer,
      offset: 0,
      length: import_buffer2.Buffer.byteLength(buffer, typeof lengthOrEncoding === "string" ? lengthOrEncoding : void 0),
...
function normalizeStreamPath(pathValue, fd) {
  ...
  if (typeof pathValue === "string" || import_buffer2.Buffer.isBuffer(pathValue)) {
    return pathValue;
  }
...
var ReadStream = class {
  ...
  async _openIfNeeded() {
    ...
    const pathStr = typeof this.path === "string" ? this.path : this.path instanceof import_buffer2.Buffer ? this.path.toString() : null;
    if (!pathStr) {
  ...
  async _readNextChunk() {
    ...
    const target = import_buffer2.Buffer.alloc(nextLength);
    this._reading = true;
    const onRead = async (error, bytesRead = 0) => {
      ...
      this.emit("data", this.readableEncoding ? chunk.toString(this.readableEncoding) : import_buffer2.Buffer.from(chunk));
      if (this._remaining === 0) {
        await this._finishReadable();
        return;
      }
  ...
  async _readAllContent() {
    ...
    return import_buffer2.Buffer.concat(chunks, totalLength);
  }
  on(event, listener) {
    const listeners = this._listeners.get(event) ?? [];
    listeners.push(listener);
...
var WriteStream = class {
  ...
  write(chunk, encodingOrCallback, callback) {
    ...
    let data;
    if (typeof chunk === "string") {
      data = import_buffer2.Buffer.from(chunk, typeof encodingOrCallback === "string" ? encodingOrCallback : "utf8");
        ...
        try {
          if (this._fileHandle) {
            ...
            if (!pathStr) {
              ...
              const chunks = this._chunks.map((chunk) => import_buffer2.Buffer.from(chunk));
              if (typeof this._position === "number") {
                ...
                const output = import_buffer2.Buffer.alloc(finalSize);
                ...
                fs.writeFileSync(
                  pathStr,
                  import_buffer2.Buffer.concat(chunks).toString(this._options?.encoding ?? "utf8")
                );
              }
...
function errnoForCode(code) {
  return Object.prototype.hasOwnProperty.call(POSIX_ERRNO, code) ? -POSIX_ERRNO[code] : -1;
}
...
function encodeBridgeBytes(value) {
  return {
    __agentOSType: "bytes",
    base64: import_buffer2.Buffer.from(value).toString("base64")
  };
}
...
async function fsReadFileAsync(path, options) {
  ...
  try {
    ...
    return import_buffer2.Buffer.from(base64Content, "base64");
  } catch (err) {
...
function normalizeFsTimeSpec(value, label) {
  if (value && typeof value === "object" && !(value instanceof Date)) {
    ...
    const sec2 = Number(value.sec);
    const nsec2 = Number(value.nsec ?? 0);
    if (!Number.isInteger(sec2)) {
      throw createInvalidArgTypeError(label, "an integer sec field", value);
    }
    if (!Number.isInteger(nsec2) || nsec2 < 0 || nsec2 >= 1e9) {
      throw createRangeError(`${label}.nsec must be an integer between 0 and 999999999`);
    }
    return { sec: sec2, nsec: nsec2 };
  }
...
var fs = {
  ...
  existsSync(path) {
    ...
    if (pathStr === "/dev/null" || pathStr === "/dev/zero" || pathStr === "/dev/urandom" || pathStr === "/dev/stdin" || pathStr === "/dev/stdout" || pathStr === "/dev/stderr") {
      return true;
    }
  ...
  readSync(fd, buffer, offset, length, position) {
    ...
    const bytes = import_buffer2.Buffer.from(base64, "base64");
  ...
  writeSync(fd, buffer, offsetOrPosition, lengthOrEncoding, position) {
    ...
    if (typeof normalized.buffer === "string") {
      dataBytes = import_buffer2.Buffer.from(normalized.buffer, normalized.encoding);
    } else {
      ...
      if (fd === 0 && (position === null || position === void 0) && typeof _kernelStdinRead !== "undefined") {
        ...
        const attemptKernelStdinRead = () => {
          _kernelStdinRead.apply(void 0, [length, 100], {
            ...
            const bytes = import_buffer2.Buffer.from(dataBase64, "base64");
    ...
    if (callback) {
      ...
      try {
        const bytesWritten = typeof normalized.buffer === "string" ? _fdWrite.applySyncPromise(
          ...
          [
            fd,
            encodeBridgeBytes(import_buffer2.Buffer.from(normalized.buffer, normalized.encoding)),
          ...
          [
            fd,
            encodeBridgeBytes(import_buffer2.Buffer.from(
              new Uint8Array(
                normalized.buffer.buffer,
                normalized.buffer.byteOffset + normalized.offset,
                normalized.length
...
function normalizeFetchRequestInit(options = {}) {
  ...
  if (normalized.body != null && normalized.duplex == null && String(normalized.method ?? "GET").toUpperCase() !== "GET" && String(normalized.method ?? "GET").toUpperCase() !== "HEAD") {
...
var SecureExecPromisesResolver = class {
  constructor() {
    this._servers = [];
  }
  cancel() {
...
var ServerResponseBridge = class {
  ...
  _appendChunk(chunk, encoding, streamed) {
    ...
    this._streamed || (this._streamed = streamed);
    this.headersSent = true;
  ...
  write(chunk, encodingOrCallback, callback) {
    ...
    if (this._streamSocket && this._chunks.length === 0 && !this.writableFinished && !this._streamedDirectly) {
...
function wakeSocketBridgeReads(socket) {
  ...
  if (!socket._bridgeReadPollTimer) {
    ...
    if (isNetBridgeMetricsEnabled() && !socket._bridgeReadPumpStarted && socket._connected && socket._refed && (hasDataListener || hasReadableListener)) {
...
var NET_BRIDGE_MAX_RAW_WRITE_BYTES = 256 * 1024;
var NetSocket = class _NetSocket {
  ...
  write(data, encodingOrCallback, callback) {
    ...
    if (this._loopbackServer || this._loopbackHttpTarget) {
      ...
      if (this._loopbackUpgradeSocket) {
        ...
        const cb22 = typeof encodingOrCallback === "function" ? encodingOrCallback : callback;
        if (cb22) cb22();
        return true;
      }
  ...
  read(size) {
    ...
    if (size == null || size <= 0) {
      const chunk2 = this._readQueue.shift() ?? null;
      if (chunk2) {
        this.readableLength = Math.max(0, this.readableLength - chunk2.length);
      }
      return chunk2;
    }
      ...
      while (!this.destroyed) {
        ...
        if (chunk === NET_BRIDGE_TIMEOUT_SENTINEL) {
          ...
          this._bridgeReadPollTimer = setTimeout(() => {
            if (isNetBridgeMetricsEnabled()) {
              const lagUs = Math.max(0, netBridgeNowUs() - scheduledAtUs - pollDelayMs * 1e3);
...
var TLSServer = class {
  ...
  _emitTlsClientError(socket, message, existingError) {
    ...
    socket.servername ?? (socket.servername = this._getClientHello(socket)?.servername);
    this._emit("tlsClientError", error, socket);
    socket.destroy();
  }
};
...
// packages/build-tools/bridge-src/builtins/os.ts
var config = {
...
function normalizeChildProcessSignal(signal) {
  ...
  if (typeof signal === "string") {
    const signalNumber = signals2[signal];
...
var CHILD_PROCESS_IPC_FRAME_PREFIX = "AGENTOS_IPC:";
...
function spawn(command, args, options) {
  ...
  if (typeof _childProcessSpawnStart !== "undefined") {
    ...
    try {
      ...
      } else if (spawnError.code == null && /ERR_NATIVE_BINARY_NOT_SUPPORTED\b/i.test(String(spawnError.message || ""))) {
...
function spawnSync(command, args, options) {
  ...
  try {
    ...
    if (err && typeof err === "object" && err.code == null && /ERR_NATIVE_BINARY_NOT_SUPPORTED\b/i.test(String(err.message || err))) {
...
var UndiciHeaders2 = undiciHeadersModule?.Headers ?? undiciHeadersModule?.default ?? undiciHeadersModule;
var UndiciRequest2 = undiciRequestModule?.Request ?? undiciRequestModule?.default ?? undiciRequestModule;
var UndiciResponse2 = undiciResponseModule?.Response ?? undiciResponseModule?.default ?? undiciResponseModule;
...
// packages/build-tools/bridge-src/builtins/buffer-constants.ts
var import_buffer22 = __toESM(require_buffer(), 1);
var BUFFER_MAX_LENGTH = typeof import_buffer22.Buffer.kMaxLength === "number" ? import_buffer22.Buffer.kMaxLength : 2147483647;
var BUFFER_MAX_STRING_LENGTH = typeof import_buffer22.Buffer.kStringMaxLength === "number" ? import_buffer22.Buffer.kStringMaxLength : 536870888;
var BUFFER_CONSTANTS = Object.freeze({
  MAX_LENGTH: BUFFER_MAX_LENGTH,
  MAX_STRING_LENGTH: BUFFER_MAX_STRING_LENGTH
});

// packages/build-tools/bridge-src/builtins/events.ts
var eventsErrorMonitor = /* @__PURE__ */ Symbol("events.errorMonitor");
...
EventEmitter.on = function on(emitter, eventName, options) {
  ...
  const iterator2 = {
    ...
    return() {
      finished = true;
      cleanup();
      for (const c of unconsumed) c.resolve({ value: void 0, done: true });
      unconsumed.length = 0;
      return Promise.resolve({ value: void 0, done: true });
    },
    throw(err) {
...
EventEmitter.addAbortListener = function addAbortListener2(signal, listener) {
...
// packages/build-tools/bridge-src/builtins/whatwg-url.ts
var _impl, _searchParams, _a;
...
var URLSearchParams2 = class _URLSearchParams {
  constructor(init) {
...
for (const name of SEARCH_PARAM_METHOD_NAMES) {
  Object.defineProperty(URLSearchParams2.prototype, name, {
    value: URLSearchParams2.prototype[name],
    writable: true,
    configurable: true,
    enumerable: true
  });
...
Object.defineProperties(URLSearchParams2.prototype, {
  size: {
    get: Object.getOwnPropertyDescriptor(URLSearchParams2.prototype, "size")?.get,
    configurable: true,
    enumerable: true
  },
  [Symbol.iterator]: {
    value: URLSearchParams2.prototype.entries,
    writable: true,
    configurable: true,
    enumerable: false
  },
  [inspectCustomSymbol]: {
    value: URLSearchParams2.prototype[inspectCustomSymbol],
    writable: true,
    configurable: true,
    enumerable: false
  },
  [toStringTagSymbol]: {
    get: Object.getOwnPropertyDescriptor(URLSearchParams2.prototype, toStringTagSymbol)?.get,
    configurable: true,
    enumerable: false
  }
});
...
var NativeURL = canUseNativeUrlImplementation(nativeUrlCandidate) ? nativeUrlCandidate : class URL2 {
  constructor(url, base) {
    ...
    if (full.startsWith("file:")) {
      ...
      const syncHrefFromSearchParams2 = () => {
...
var URL22 = (_a = class {
  constructor(input, base) {
    ...
    try {
      __privateSet(this, _impl, normalizedArgs.base !== void 0 ? new NativeURL(normalizedArgs.input, normalizedArgs.base) : new NativeURL(normalizedArgs.input));
    } catch {
  ...
  get href() {
    ...
    return __privateGet(this, _impl).href;
  }
  set href(value) {
    __privateGet(this, _impl).href = toNodeUSVString(value);
  }
  get origin() {
    return __privateGet(this, _impl).origin;
  }
  get protocol() {
    return __privateGet(this, _impl).protocol;
  }
  set protocol(value) {
    __privateGet(this, _impl).protocol = toNodeUSVString(value);
  }
  get username() {
    return __privateGet(this, _impl).username;
  }
  set username(value) {
    __privateGet(this, _impl).username = toNodeUSVString(value);
  }
  get password() {
    return __privateGet(this, _impl).password;
  }
  set password(value) {
    __privateGet(this, _impl).password = toNodeUSVString(value);
  }
  get host() {
    return __privateGet(this, _impl).host;
  }
  set host(value) {
    __privateGet(this, _impl).host = toNodeUSVString(value);
  }
  get hostname() {
    return __privateGet(this, _impl).hostname;
  }
  set hostname(value) {
    __privateGet(this, _impl).hostname = toNodeUSVString(value);
  }
  get port() {
    return __privateGet(this, _impl).port;
  }
  set port(value) {
    __privateGet(this, _impl).port = toNodeUSVString(value);
  }
  get pathname() {
    return __privateGet(this, _impl).pathname;
  }
  set pathname(value) {
    __privateGet(this, _impl).pathname = toNodeUSVString(value);
  }
  get search() {
    if (!(this instanceof _a)) {
      throw createUrlReceiverTypeError();
    }
    return __privateGet(this, _impl).search;
  }
  set search(value) {
    __privateGet(this, _impl).search = toNodeUSVString(value);
  }
  get searchParams() {
    if (!__privateGet(this, _searchParams)) {
      __privateSet(this, _searchParams, new URLSearchParams2({
        [kLinkedSearchParams]: () => __privateGet(this, _impl).searchParams
      }));
    }
    return __privateGet(this, _searchParams);
  }
  get hash() {
    return __privateGet(this, _impl).hash;
  }
  set hash(value) {
    __privateGet(this, _impl).hash = toNodeUSVString(value);
  }
  toString() {
    if (!(this instanceof _a)) {
      throw createUrlReceiverTypeError();
  ...
  [inspectCustomSymbol](depth, options, inspect) {
    const inspectName = this.constructor === _a ? "URL" : this.constructor.name;
    if (depth < 0) {
      return `${inspectName} {}`;
    }
    const formatValue = typeof inspect === "function" ? (value) => inspect(value, options) : (value) => JSON.stringify(value);
...
}, _impl = new WeakMap(), _searchParams = new WeakMap(), _a);
for (const name of ["toString", "toJSON"]) {
  Object.defineProperty(URL22.prototype, name, {
    value: URL22.prototype[name],
    writable: true,
    configurable: true,
    enumerable: true
  });
...
for (const name of [
  ...
  const descriptor = Object.getOwnPropertyDescriptor(URL22.prototype, name);
  if (!descriptor) {
    continue;
  }
  descriptor.enumerable = true;
  Object.defineProperty(URL22.prototype, name, descriptor);
}
Object.defineProperties(URL22.prototype, {
  [inspectCustomSymbol]: {
    value: URL22.prototype[inspectCustomSymbol],
    writable: true,
    configurable: true,
    enumerable: false
  },
  [toStringTagSymbol]: {
    get: Object.getOwnPropertyDescriptor(URL22.prototype, toStringTagSymbol)?.get,
    configurable: true,
    enumerable: false
  }
});
for (const name of ["canParse", "createObjectURL", "revokeObjectURL"]) {
  Object.defineProperty(URL22, name, {
    value: URL22[name],
    writable: true,
    configurable: true,
    enumerable: true
...
// packages/build-tools/bridge-src/builtins/module-loader.ts
...
function assertCommonjsLoadable(filename) {
  if (typeof _moduleFormat === "undefined" || typeof _moduleFormat.applySyncPromise !== "function") {
...
function ensureBuiltinEventsStdlibModule() {
  ...
  builtinEventsConstructor = typeof builtinEventsStdlibModule === "function" ? builtinEventsStdlibModule : builtinEventsStdlibModule?.EventEmitter;
...
if (typeof builtinStreamStdlibModule?.Stream === "function") {
  ...
  const isBuiltinStreamInstance = (value) => {
    ...
    return typeof builtinStreamStdlibModule.Readable === "function" && value instanceof builtinStreamStdlibModule.Readable || typeof builtinStreamStdlibModule.Writable === "function" && value instanceof builtinStreamStdlibModule.Writable || typeof builtinStreamStdlibModule.Duplex === "function" && value instanceof builtinStreamStdlibModule.Duplex || typeof builtinStreamStdlibModule.Transform === "function" && value instanceof builtinStreamStdlibModule.Transform || typeof builtinStreamStdlibModule.PassThrough === "function" && value instanceof builtinStreamStdlibModule.PassThrough;
  };
...
function ensureBuiltinUrlStdlibModule() {
  ...
  builtinUrlStdlibModule.URL = URL22;
  builtinUrlStdlibModule.URLSearchParams = URLSearchParams2;
  ...
  if (builtinUrlStdlibModule?.default && typeof builtinUrlStdlibModule.default === "object") {
    builtinUrlStdlibModule.default.URL = URL22;
    builtinUrlStdlibModule.default.URLSearchParams = URLSearchParams2;
...
var __jsRuntimeBuiltinAllowlist = null;
function rejectRestrictedBuiltinRequest(request) {
  ...
  if (Array.isArray(allow)) {
    const root = String(normalized == null ? request : normalized).replace(/^node:/, "").split("/")[0];
...
exposeCustomGlobal("__agentOSInitJsRuntime", function(allowlist) {
  __jsRuntimeBuiltinAllowlist = Array.isArray(allowlist) ? allowlist.map((name) => String(name).replace(/^node:/, "").split("/")[0]) : null;
});
...
// packages/build-tools/bridge-src/builtins/process.ts
...
var bufferPolyfillMutable = import_buffer23.Buffer;
...
var bufferProto = import_buffer23.Buffer.prototype;
...
var bufferCtorMutable = import_buffer23.Buffer;
...
function _addListener(event, listener, once3 = false) {
  const target = once3 ? _processOnceListeners : _processListeners;
  if (!target[event]) {
    target[event] = [];
  }
...
function emitListeners(listeners, onceListeners, event, args) {
  ...
  const once3 = onceListeners[event] ? onceListeners[event].slice() : [];
  if (once3.length > 0) {
    onceListeners[event] = [];
  }
  ...
  for (const listener of once3) {
    listener(...args);
  }
  return persistent.length + once3.length > 0;
}
function createStdioWriteStream(options) {
  ...
  const stream = {
    write(data, encodingOrCallback, callback) {
      if (data instanceof Uint8Array || typeof import_buffer23.Buffer !== "undefined" && import_buffer23.Buffer.isBuffer(data)) {
        options.write(data);
      } else {
...
var defaultConsole = new Console();
...
var VM_CONTEXT_TAG = typeof Symbol === "function" ? /* @__PURE__ */ Symbol.for("secure-exec.vm.context") : "__secure_exec_vm_context__";
var VM_CONTEXT_ID = typeof Symbol === "function" ? /* @__PURE__ */ Symbol.for("secure-exec.vm.context.id") : "__secure_exec_vm_context_id__";
...
var WorkerThreadPort = class extends EventEmitter {
  postMessage() {
  }
  start() {
  }
...
var WorkerThreadMessageChannel = class {
  constructor() {
    this.port1 = new WorkerThreadPort();
    this.port2 = new WorkerThreadPort();
  }
};
var WorkerThreadWorker = class extends EventEmitter {
  constructor() {
    super();
...
var builtinWorkerThreadsModule = {
  ...
  SHARE_ENV: /* @__PURE__ */ Symbol.for("secure-exec.worker_threads.SHARE_ENV"),
...
function flushLiveStdinBuffer() {
  ...
  _stdinLiveBuffer = "";
  const data = _stdin.encoding ? chunk : import_buffer23.Buffer.from(chunk);
    ...
    try {
      while (!getStdinEnded()) {
        ...
        _stdinLiveBuffer += _stdinLiveDecoder.decode(
          import_buffer23.Buffer.from(dataBase64, "base64"),
          { stream: true }
        );
        flushLiveStdinBuffer();
      }
...
function stdinDispatch(eventType, payload) {
  ...
  if (payload && typeof payload === "object" && typeof payload.dataBase64 === "string") {
    const bytes = import_buffer23.Buffer.from(payload.dataBase64, "base64");
    if (bytes.length === 0) {
      return;
    }
    ...
    chunk = typeof payload === "string" ? payload : payload == null ? "" : import_buffer23.Buffer.from(payload).toString("utf8");
  }
  if (!chunk) {
    return;
  }
  ...
  if (binary && !_stdin.encoding && getStdinFlowMode()) {
    ...
    _stdinLiveBuffer = "";
    emitStdinListeners("data", import_buffer23.Buffer.from(buffered, "latin1"));
    maybeEmitLiveStdinTerminalEvents();
    return;
  }
  flushLiveStdinBuffer();
...
var builtinTtyModule = {
  ReadStream: class ReadStream4 {
    constructor(fd) {
      return TtyReadStream(fd);
    }
  },
  WriteStream: class WriteStream4 {
    constructor(fd) {
...
function getNodeReadableAsyncIterable(stream) {
  if (!stream || typeof stream.on !== "function" || typeof stream.read !== "function" && typeof stream.pipe !== "function" && typeof stream.resume !== "function") {
    return null;
  }
  return {
    async *[Symbol.asyncIterator]() {
      ...
      let error = null;
      const cleanup = [];
      const removeListener = typeof stream.off === "function" ? stream.off.bind(stream) : typeof stream.removeListener === "function" ? stream.removeListener.bind(stream) : null;
      const flush = () => {
        while (pendingResolves.length > 0) {
          if (error) {
            pendingResolves.shift()?.(Promise.reject(error));
...
var builtinStreamPromisesModule = {
  ...
  async pipeline(source, destination) {
    const readable = getNodeReadableAsyncIterable(source) ?? (source && typeof source[Symbol.asyncIterator] === "function" ? source : source && typeof source.getReader === "function" ? {
      async *[Symbol.asyncIterator]() {
...
var Buffer3 = import_buffer23.Buffer;
...
var SandboxDOMException = class extends Error {
...
function isIntegerTypedArray(value) {
  ...
  return value instanceof Int8Array || value instanceof Int16Array || value instanceof Int32Array || value instanceof Uint8Array || value instanceof Uint16Array || value instanceof Uint32Array || value instanceof Uint8ClampedArray || value instanceof BigInt64Array || value instanceof BigUint64Array || import_buffer23.Buffer.isBuffer(value);
}
function toBase64(data) {
  if (typeof data === "string") {
    return import_buffer23.Buffer.from(data).toString("base64");
  }
  if (data instanceof ArrayBuffer) {
    return import_buffer23.Buffer.from(new Uint8Array(data)).toString("base64");
  }
  if (ArrayBuffer.isView(data)) {
...
function toArrayBuffer(data) {
  const buf = import_buffer23.Buffer.from(data, "base64");
...
function normalizeBridgeAlgorithm(algorithm) {
  ...
  if (publicExponent && ArrayBuffer.isView(publicExponent)) {
    normalized.publicExponent = import_buffer23.Buffer.from(
      new Uint8Array(
...
var SandboxCrypto = class {
  ...
  getRandomValues(array) {
    ...
    try {
      ...
      const hostBytes = import_buffer23.Buffer.from(base64, "base64");
...
function createBuiltinHash(algorithm) {
  ...
  return {
    update(data, encoding) {
      const buffer = typeof data === "string" ? import_buffer23.Buffer.from(data, encoding || "utf8") : import_buffer23.Buffer.from(data);
      chunks.push(buffer);
      return this;
    },
    digest(encoding) {
      ...
      const input = chunks.length === 1 ? chunks[0] : import_buffer23.Buffer.concat(chunks);
      const resultBase64 = _cryptoHashDigest.applySync(void 0, [
        String(algorithm),
        input.toString("base64")
      ]);
      const result = import_buffer23.Buffer.from(String(resultBase64 || ""), "base64");
      return encoding ? result.toString(encoding) : result;
    }
  };
}
function toSymmetricKeyBuffer(key) {
  if (isBuiltinKeyObject(key)) {
    ...
    return import_buffer23.Buffer.from(String(key._serialized.raw || ""), "base64");
  }
  if (key && typeof key === "object" && key[Symbol.toStringTag] === "CryptoKey" && typeof key._raw === "string") {
    return import_buffer23.Buffer.from(String(key._raw || ""), "base64");
  }
  return typeof key === "string" ? import_buffer23.Buffer.from(key) : toCryptoBuffer(key);
}
...
function isBufferLikeValue(value) {
  return import_buffer23.Buffer.isBuffer(value) || value instanceof ArrayBuffer || ArrayBuffer.isView(value);
}
function toCryptoBuffer(value, encoding = void 0) {
  if (import_buffer23.Buffer.isBuffer(value)) {
    return import_buffer23.Buffer.from(value);
  }
  if (typeof value === "string") {
    return import_buffer23.Buffer.from(value, encoding || "utf8");
  }
  if (value instanceof ArrayBuffer) {
    return import_buffer23.Buffer.from(new Uint8Array(value));
  }
  if (ArrayBuffer.isView(value)) {
    return import_buffer23.Buffer.from(new Uint8Array(value.buffer, value.byteOffset, value.byteLength));
  }
  return import_buffer23.Buffer.from(value ?? []);
}
...
function deserializeBridgeValue(value) {
  ...
  if (value.__type === "buffer") {
    return import_buffer23.Buffer.from(String(value.value || ""), "base64");
  }
...
var BuiltinKeyObject = class {
  ...
  constructor(serialized, token) {
    ...
    this.symmetricKeySize = serialized.raw ? import_buffer23.Buffer.from(String(serialized.raw), "base64").length : void 0;
  ...
  export(options = void 0) {
    if (this.type === "secret") {
      ...
      return import_buffer23.Buffer.from(String(this._serialized.raw || ""), "base64");
    }
...
var BuiltinCipherTransform = class {
  _mode;
  _algorithm;
  _key;
  _iv;
  ...
  update(data, inputEncoding = void 0, outputEncoding = void 0) {
    ...
    return encodeCryptoResult(import_buffer23.Buffer.from(String(resultBase64 || ""), "base64"), outputEncoding);
  }
  final(outputEncoding = void 0) {
    ...
    if (response.authTag) {
      this._authTag = import_buffer23.Buffer.from(String(response.authTag), "base64");
    }
    return encodeCryptoResult(import_buffer23.Buffer.from(String(response.data || ""), "base64"), outputEncoding);
  }
  ...
  getAuthTag() {
    ...
    return import_buffer23.Buffer.from(this._authTag);
  }
...
var BuiltinSignContext = class {
  ...
  _inputBuffer() {
    if (this._chunks.length === 0) {
      return import_buffer23.Buffer.alloc(0);
    }
    return this._chunks.length === 1 ? this._chunks[0] : import_buffer23.Buffer.concat(this._chunks);
  }
...
var BuiltinVerifyContext = class extends BuiltinSignContext {
...
var diffieHellmanSessionFinalizer = typeof FinalizationRegistry === "function" ? new FinalizationRegistry((sessionId) => {
  try {
    callCryptoSync(_cryptoDiffieHellmanSessionDestroy, "createDiffieHellman", [sessionId]);
  } catch {
  }
}) : null;
var BuiltinDiffieHellmanSession = class {
  _sessionId;
  constructor(request) {
...
var builtinCryptoModule = {
  ...
  publicEncrypt(key, buffer) {
    ...
    return import_buffer23.Buffer.from(String(payload || ""), "base64");
  },
  ...
  randomBytes(size) {
    ...
    return import_buffer23.Buffer.from(bytes);
  },
  randomUUID() {
...
function pathToFileURL2(filePath) {
  ...
  return new URL22(`file://${pathname.startsWith("/") ? pathname : `/${pathname}`}`);
}
function fileURLToPath2(input) {
  const href = input instanceof URL22 ? input.href : String(input ?? "");
...
function installBuiltinUtilFormatWithOptions(builtinUtilModule2) {
  if (!builtinUtilModule2 || typeof builtinUtilModule2.formatWithOptions === "function") {
    return builtinUtilModule2;
  }
  builtinUtilModule2.formatWithOptions = function formatWithOptions(inspectOptions, format, ...args) {
    const inspectValue = (value) => {
      if (typeof builtinUtilModule2.inspect === "function") {
        return builtinUtilModule2.inspect(value, inspectOptions);
      }
      try {
        return JSON.stringify(value);
      } catch {
...
function setupGlobals() {
  ...
  const builtinUtilModule2 = globalThis.__secureExecBuiltinUtilModule;
  if (builtinUtilModule2?.types) {
    builtinUtilModule2.types.isProxy = () => false;
  }
  installBuiltinUtilFormatWithOptions(builtinUtilModule2);
  ...
  g.fetch = fetch;
  g.Headers = UndiciHeaders2;
  g.Request = UndiciRequest2;
  g.Response = UndiciResponse2;
  installSafeIntlFormatters(g);
}

// packages/build-tools/bridge-src/builtins/active-handles.ts
var HANDLE_DISPATCH = {
...
// packages/build-tools/bridge-src/index.ts
var index_exports = {};
