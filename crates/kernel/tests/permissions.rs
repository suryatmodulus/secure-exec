...
fn kernel_mounts_require_write_permission_on_the_mount_path() {
    ...
    assert_eq!(
        checked
            ...
            .as_slice(),
        [(
            secure_exec_kernel::permissions::FsOperation::Write,
    ...
    config.permissions = Permissions {
        filesystem: Some(Arc::new(move |request: &FsAccessRequest| {
            checked_for_permission
                ...
                .push((request.op, request.path.clone()));
            match request.op {
                secure_exec_kernel::permissions::FsOperation::Write => PermissionDecision::allow(),
                secure_exec_kernel::permissions::FsOperation::MountSensitive => {
