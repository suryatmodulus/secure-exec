...
// Build-foundation shim: the secure-exec docs CONTENT still imports
// `@astrojs/starlight/components`. Map that path onto a thin shim that
// re-exports the de-Starlighted theme's MDX primitives so content builds
// unchanged. Phase 2 rewrites the content to import the theme directly and
// drops this alias.
const starlightShim = fileURLToPath(
	new URL("./src/lib/starlight-shim.jsx", import.meta.url),
);
...
export default defineConfig({
	...
	vite: {
		resolve: {
			...
			alias: {
				"@astrojs/starlight/components": starlightShim,
			},
		},
		optimizeDeps: {
