...
- **owned-sidecar**: `NodeRuntime.create()` boots a fresh sidecar process for each runtime. This is the default, fully isolated path (see [Process Isolation](/docs/process-isolation)).
...
See [Process Isolation](/docs/process-isolation) for what each of these shares and isolates.
